...
from app.read_advisors import Read
...
@app.route("/read_advisors", methods=["POST"])
def read_advisors():
...
@app.route("/insert_advisors", methods=["PUT"])
def insert_advisors():
    ...
    if request.method == "PUT":
        ...
        flag, errors = controller.run(advisors=body)
        if not flag: return jsonify(errors), 422
...
@app.route("/update_advisors", methods=["PATCH"])
def update_advisors():
...
@app.route("/delete_advisors", methods=["DELETE"])
def delete_advisors():
