...
from app.read_products import Read
...
@app.route("/insert_products", methods=["PUT"])
def insert_products():
    ...
    if request.method == "PUT":
        ...
        flag, errors = controller.run(products=body)
        if not flag: return jsonify(errors), 422
...
@app.route("/update_products", methods=["PATCH"])
def update_products():
