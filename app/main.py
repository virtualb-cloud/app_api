...
from app.read_portfolios import Read
...
@app.route("/read_portfolios", methods=["POST"])
def read_portfolios():
...
@app.route("/insert_portfolios", methods=["PUT"])
def insert_portfolios():
    ...
    if request.method == "PUT":
        ...
        flag, errors = controller.run(portfolios=body)
        if not flag: return jsonify(errors), 422
...
@app.route("/update_portfolios", methods=["PATCH"])
...
@app.route("/delete_portfolios", methods=["DELETE"])
